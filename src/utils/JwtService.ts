...
/**
 * 🔹 Lấy role của user từ AuthContext
 */
export function getRole(authContext: ReturnType<typeof useAuth>): string | null {
  return authContext.userInfo?.role || null;
}

/**
 * 🔹 Lấy avatar user
 */
export function getAvatar(authContext: ReturnType<typeof useAuth>): string | null {
  return authContext.userInfo?.avatar || null;
}

/**
 * 🔹 Lấy ID user
 */
export function getIdUser(authContext: ReturnType<typeof useAuth>): number | null {
  return authContext.userInfo?.id || null;
}

/**
 * 🔹 Kiểm tra đã đăng nhập
 */
export function isAuthenticated(authContext: ReturnType<typeof useAuth>): boolean {
 ...
 * 🔹 Đăng xuất
 */
export async function logout(authContext: ReturnType<typeof useAuth>, navigate?: NavigateFunction) {
